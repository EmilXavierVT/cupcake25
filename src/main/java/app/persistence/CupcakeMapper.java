...
public class CupcakeMapper {
    ...
    private final String GET_ALL_BOTTOMS = "SELECT * FROM public.the_bottoms";
    private final String GET_ALL_ICINGS = "SELECT * FROM public.icing";
    ...
    public List<Bottom> getAllBottoms() throws SQLException {
        ...
        try (Connection connection = connectionPool.getConnection()) {
            try (PreparedStatement ps = connection.prepareStatement(GET_ALL_BOTTOMS)) {
    ...
    public Bottom getBottomById(int id) throws SQLException {
        Bottom bottom = null;
        try (Connection connection = connectionPool.getConnection()) {
            String sql = "SELECT * FROM public.the_bottoms WHERE bottom_id = ?";
    ...
    public Icing getIcingById(int id) throws SQLException {
        Icing icing = null;
        try (Connection connection = connectionPool.getConnection()) {
            String sql = "SELECT * FROM public.icing WHERE icing_id = ?";
            try (PreparedStatement ps = connection.prepareStatement(sql)) {
                ps.setInt(1, id);
                ResultSet rs = ps.executeQuery();
                if (rs.next()) {
    ...
    public UserDefinedCupcake createUserDefinedCupcake(int bottom, int icing) throws SQLException {
        ...
        try (Connection connection = connectionPool.getConnection()) {
            String sql = "INSERT INTO public.user_defined_cupcake (bottom_id, icing_id) VALUES (?, ?) RETURNING id";
            try (PreparedStatement ps = connection.prepareStatement(sql)) {
                ps.setInt(1, bottom);
                ps.setInt(2, icing);
                ResultSet rs = ps.executeQuery();
